  ...
  bot.onText(/\/start/, async (msg) => {
    ...
    try {
      ...
      if (!created) {
        ...
        const updateFields = {};
        if (username !== user.username) updateFields.username = username || null;
        if (first_name !== user.first_name) updateFields.first_name = first_name || null;
        if (last_name !== user.last_name) updateFields.last_name = last_name || null;

        if (Object.keys(updateFields).length > 0) {
          await user.update(updateFields);
        }
      } else {
        // Если пользователь новый, назначаем ему бесплатную подписку
